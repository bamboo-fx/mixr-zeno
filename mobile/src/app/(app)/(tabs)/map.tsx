...
import { api, type VenueCategory } from '@/lib/api';
import { useAuthStore } from '@/lib/state/auth-store';
import { GelBackground } from '@/components/gel/GelBackground';
...
export default function MapScreen() {
  ...
  const collegeId = useAuthStore((s) => s.profile?.collegeId);
  const profileId = useAuthStore((s) => s.profile?.id);

  const [hours, setHours] = useState<number>(2);
  const [viewMode, setViewMode] = useState<'map' | 'list'>('list');
  const [selectedVenueId, setSelectedVenueId] = useState<string | null>(null);
  const [submitPanelOpen, setSubmitPanelOpen] = useState<boolean>(false);
  ...
  const { data: canSubmitData } = useQuery({
    queryKey: ['heatmap-can-submit', profileId],
    queryFn: () => api.heatmap.canSubmit(profileId!),
    enabled: !!profileId,
    ...
    <GelBackground>
      ...
      <View style={[styles.header, { paddingTop: insets.top + 10 }]}>
        <View style={styles.headerContent}>
          <Image
            source={require('../../../../assets/images/mixr-logo-new.png')}
            style={styles.logo}
            resizeMode="contain"
          />
          <View style={styles.headerActions}>
            <Pressable
              onPress={() => { Haptics.tap(); setViewMode(v => v === 'map' ? 'list' : 'map'); }}
              style={styles.viewToggle}
            >
              {viewMode === 'map'
                ? <List size={18} color={DS.Color.text} />
                : <MapIcon size={18} color={DS.Color.text} />
              }
            </Pressable>
          </View>
        </View>
        ...
        <View style={styles.subtitleRow}>
          ...
          <Text style={styles.subtitleText}>Campus Heat</Text>
          <Text style={styles.subtitleDot}>·</Text>
          <Text style={styles.subtitleSub}>Where the vibes are at</Text>
        </View>
      </View>
            ...
            {heatmapData.heatmap.map((venue, index) => {
              ...
              return (
                ...
                >
                  <View style={[
                    styles.venueCard,
                    index === 0 && styles.venueCardTop,
                    selectedVenueId === venue.venueId && styles.venueCardSelected,
                  ]}>
                    {/* Rank */}
                    ...
                    <View style={styles.heatBarWrap}>
                      <View style={styles.heatBarBg}>
                        <LinearGradient
                          ...
                          style={[styles.heatBarFill, { width: `${Math.max(12, venue.normalizedIntensity * 100)}%` as any }]}
                        />
                      </View>
                    </View>
                  </View>
        ...
        {submitPanelOpen && canSubmit && (
          <Animated.View style={[styles.venuePicker, panelStyle]}>
            ...
            <ScrollView
              ...
              contentContainerStyle={styles.venuePickerScroll}
              style={{ flexGrow: 0 }}
            >
              {venues.map((venue) => {
                const Icon = CATEGORY_ICONS[venue.category] ?? HelpCircle;
